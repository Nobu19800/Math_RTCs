  ...
  : public DynamicComponentBase
{
 public:
   ...
   */
  Saturation(RTC::Manager* manager);
 ...
 protected:
  ...
  /*!
   * データ上限値
   * - Name: UpperLimit UpperLimit
   * - DefaultValue: 10
   */
  std::string m_UpperLimit;
  /*!
   * データ下限値
   * - Name: LowerLimit LowerLimit
   * - DefaultValue: -10
   */
...
class SaturationCompBase : public RTCLogicBase
{
public:
	virtual void setLimit(std::string ul, std::string ll){};
...
class SaturationSingle : public SaturationCompBase
{
public:
	SaturationSingle(Saturation *rtc)
	{
	...
	void setLimit(std::string ul, std::string ll)
	{
		std::vector<T2> tmp;
		if(getValueInString(ul,tmp))
		{
			upperLimit = tmp[0];
		}
		if(getValueInString(ll,tmp))
		{
			lowerLimit = tmp[0];
		}
	}
...
template <class T, class T2>
class SaturationSeq : public SaturationCompBase
{
public:
	SaturationSeq(Saturation *rtc)
	{
	...
	RTC::ReturnCode_t onExecute(RTC::UniqueId ec_id){
		if(m_inport->isNew())
		{
			...
			for(int i=0;i < data.size();i++)
			{
				T2 ul;
				T2 ll;
				if(!getValueInList(upperLimit,i,ul))return RTC::RTC_ERROR;
				if(!getValueInList(lowerLimit,i,ll))return RTC::RTC_ERROR;
				//std::cout << ul << "\t" << ll << std::endl;
				T2 val = RTSaturation<T2>(data[i],ul,ll);
				ans.push_back(val);
	...
	void setLimit(std::string ul, std::string ll)
	{
		getValueInString(ul,upperLimit);

		getValueInString(ll,lowerLimit);
	...
	std::vector<T2> upperLimit;
	std::vector<T2> lowerLimit;

};

extern "C"
{
  DLL_EXPORT void SaturationInit(RTC::Manager* manager);
};
