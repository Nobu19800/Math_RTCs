  ...
  : public DynamicComponentBase
{
 public:
   ...
   */
  SqrtFunction(RTC::Manager* manager);
...
class SqrtFunctionSingle : public RTCLogicBase
{
public:
	SqrtFunctionSingle(SqrtFunction *rtc)
	{
	...
	RTC::ReturnCode_t onExecute(RTC::UniqueId ec_id){
		if(m_inport->isNew())
		{
			T2 ans = RTSignedSqrt<T2>(m_inport->getData());
...
template <class T, class T2>
class SqrtFunctionSeq : public RTCLogicBase
{
public:
	SqrtFunctionSeq(SqrtFunction *rtc)
	{
...
extern "C"
{
  DLL_EXPORT void SqrtFunctionInit(RTC::Manager* manager);
};
