  ...
  : public DynamicComponentBase
{
 public:
   ...
   */
  AbsFunction(RTC::Manager* manager);
...
class AbsFunctionSingle : public RTCLogicBase
{
public:
	AbsFunctionSingle(AbsFunction *rtc)
	{
	...
	RTC::ReturnCode_t onExecute(RTC::UniqueId ec_id){
		if(m_inport->isNew())
		{
			T2 ans = std::abs((T2)m_inport->getData());
...
template <class T, class T2>
class AbsFunctionSeq : public RTCLogicBase
{
public:
	AbsFunctionSeq(AbsFunction *rtc)
	{
...
extern "C"
{
  DLL_EXPORT void AbsFunctionInit(RTC::Manager* manager);
};
